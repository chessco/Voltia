  ...
  return (
    <div className="space-y-10">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900">{t('inventory.title')}</h1>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          <div className="flex items-center gap-4">
            ...
            <select className="bg-white border border-gray-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all">
              ...
              <option>{t('products.categories.powerDistribution')}</option>
              <option>{t('products.categories.monitoring')}</option>
              <option>{t('products.categories.protection')}</option>
            </select>
          </div>
          <div className="text-sm text-gray-500 font-medium">
            <Trans i18nKey="inventory.showingProducts" count={PRODUCTS.length}>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="px-8 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t('inventory.table.product')}</th>
                ...
                <th className="px-8 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center">{t('inventory.table.stock')}</th>
                <th className="px-8 py-4 text-[10px) font-bold text-gray-400 uppercase tracking-widest">{t('inventory.table.price')}</th>
                <th className="px-8 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t('inventory.table.status')}</th>
                <th className="px-8 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-right">{t('inventory.table.actions')}</th>
              </tr>
            </thead>
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-gray-700">
                        {product.status === 'In Stock' ? t('inventory.status.inStock') : 
                         product.status === 'Low Stock' ? t('inventory.status.lowStock') : t('inventory.status.outOfStock')}
                      </span>
                    </div>
                  </td>
        ...
        <div className="p-6 border-t border-gray-100 flex items-center justify-between bg-gray-50/30">
          <div className="flex items-center gap-2">
            <button className="px-3 py-1.5 border border-gray-200 rounded-lg text-xs font-bold text-gray-400 hover:bg-white hover:text-gray-900 transition-all disabled:opacity-50" disabled>{t('common.previous')}</button>
            ...
            <button className="px-3 py-1.5 border border-gray-200 rounded-lg text-xs font-bold text-gray-500 hover:bg-white hover:text-gray-900 transition-all">{t('common.next')}</button>
          </div>
          <p className="text-xs text-gray-400 font-medium tracking-wider uppercase">{t('common.pageOf', { current: 1, total: 12 })}</p>
        </div>
      </div>
    </div>
  );
